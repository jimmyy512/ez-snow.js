class ezSnow {
  constructor(appendDomName = "body", srcPath , bgWidth,bgHeight) {
  ...
  init() {
    ...
    for (let i = 0; i < this._imgSum; i++) {
      ...
      {
        img.style.width=`${this._bgWidth}px`;
        img.style.height=`${this._bgHeight}px`;
      }
  ...
  setAutoScale(RandScaleMode,ScaleMax=this._ScaleMax,ScaleMin=this._ScaleMin)
  {
  ...
  setRandomPhysics(isPhyRandom,Vx,Vy)
  {
    if(typeof isPhyRandom!=="boolean")
      this._warnTip("setRandomPhysics","param isPhyRandom need boolean.");
    if(isPhyRandom)
    {
      if(
        !Array.isArray(Vx) ||
      ...
      {
        this._warnTip("setRandomPhysics","Vx Vy need Array param, Vx:[MaxRandom,MinRandom]  Vy:[MaxRandom,MinRandom].")
        return;
      }
    ...
    else
    {
      if(
        ...
        typeof Vy ===undefined)
      {
        this._warnTip("setRandomPhysics","Vx Vy is undefined.")
        return;
      }
  ...
  setSnowNum(snowSum)
  {
    ...
    {
      this._warnTip("setSnowNum","param need number ");
      return;
    }
